...
var initCmd = &cobra.Command{
	Use:   "init",
	Short: "Initialize Forge for first use",
	Long: `Initialize Forge by setting up configuration, database, and local node.
...
func printInitSummary(results []initResult, err error) error {
	...
	if err != nil {
		...
		fmt.Println("  - Ensure ~/.config/forge is writable")
		fmt.Println("  - Ensure ~/.local/share/forge is writable")
		fmt.Println()
		return err
	}
	...
	fmt.Println("  1. Edit config at ~/.config/forge/config.yaml")
	fmt.Println("  2. Add API keys for your AI providers")
	fmt.Println("  3. Create a workspace: forge ws create --path /path/to/repo")
	fmt.Println("  4. Launch the TUI: forge")
	fmt.Println()
...
const configTemplate = `# Forge Configuration File
# Generated by 'forge init'

# Global settings
global:
  # Directory for Forge data (database, logs, transcripts)
